...
echo "\$accentLight: #FFFFFF" >> abstracts/_colours.sass
echo "\$accentDark: #000000" >> abstracts/_colours.sass
echo "\$midtoneLight: #FFFFFF" >> abstracts/_colours.sass
echo "\$midtoneDark: #000000" >> abstracts/_colours.sass
echo $'\n$error: #e84610' >> abstracts/_colours.sass
echo $'\n$success: #197019' >> abstracts/_colours.sass
...
echo "\$greyTransparent: rgba(0,0,0,0.2)" >> abstracts/_colours.sass
echo $'\n// Fifty shades of ...' >> abstracts/_colours.sass
echo "\$grey: #999999" >> abstracts/_colours.sass
echo "\$greyDark: #333333" >> abstracts/_colours.sass
echo "\$greyLight: #f4f2f2" >> abstracts/_colours.sass
echo "\$greyHelp: #808080" >> abstracts/_colours.sass
echo "\$greyInvalid: #aab7c4" >> abstracts/_colours.sass
...
echo $'$boxShadowDark: 0 1.5rem 4rem rgba($black, .55)' >> abstracts/_variables.sass
echo $'\n$bodyPadding: 0' >> abstracts/_variables.sass
echo $'\n$cardGutter: 20px' >> abstracts/_variables.sass
echo $'\n$small: 80%' >> abstracts/_variables.sass
echo $'\n$borderRadius: 3px' >> abstracts/_variables.sass
echo $'$greyBorder: 1px solid $greyLight' >> abstracts/_variables.sass
...
echo "" >> base/_base.sass
echo $'\t/* Mobile devices do not honour the "vh" property properly */' >> base/_base.sass
echo $'\t/* So setting the view height to less than 100 for mobile devices */' >> base/_base.sass
echo $'\tmin-height: 92vh' >> base/_base.sass
echo $'\t@include respond(desktop)' >> base/_base.sass
...
echo $'\n// Adjusted from https://www.powdertothepeople.tv/2015/06/29/functional-scss/' >> abstracts/_mixins.sass
echo $'// NB Equilateral triangle if base is 0.86 x height' >> abstracts/_mixins.sass
echo "@mixin arrow(\$height, \$base, \$color, \$direction)" >> abstracts/_mixins.sass
echo $'\twidth: 0' >> abstracts/_mixins.sass
echo $'\theight: 0' >> abstracts/_mixins.sass
echo $'\tborder: $base solid transparent' >> abstracts/_mixins.sass
echo $'\t@if $direction == "down"' >> abstracts/_mixins.sass
echo $'\t\tborder-top: $height solid $color' >> abstracts/_mixins.sass
echo $'\t@if $direction == "up"' >> abstracts/_mixins.sass
echo $'\t\tborder-bottom: $height solid $color' >> abstracts/_mixins.sass
echo $'\t@if $direction == "left"' >> abstracts/_mixins.sass
echo $'\t\tborder-right: $height solid $color' >> abstracts/_mixins.sass
echo $'\t@if $direction == "right"' >> abstracts/_mixins.sass
echo $'\t\tborder-left: $height solid $color' >> abstracts/_mixins.sass
echo $'\n@mixin cardWidth(\$numCards, \$gap)' >> abstracts/_mixins.sass
echo $'\t// Will not have a gap afer the last card in a row' >> abstracts/_mixins.sass
echo $'\t// So calculate the portion of the gap to allocate to each card' >> abstracts/_mixins.sass
echo $'\t$cardGap: $gap * ($numCards - 1) / $numCards' >> abstracts/_mixins.sass
echo $'\t// Calculate the width of each card' >> abstracts/_mixins.sass
echo $'\t// based on how many card in a row and the gap allocated to each card' >> abstracts/_mixins.sass
echo $'\tflex: 0 1 calc(100%/#{$numCards} - #{$cardGap})' >> abstracts/_mixins.sass
...
echo $'// Notebook\t\t\t\t\t951 - 1200\t\tdesktop' >> abstracts/_media-queries.sass
echo $'// Desktop\t\t\t\t1200 - 1499\t\t\t\tXL' >> abstracts/_media-queries.sass
echo $'// XL Desktop\t\t\t\t1500+\t\t\t\tXL' >> abstracts/_media-queries.sass
...
echo $'\n$breakpoints:('tabletPortrait': (min-width: 37.5em), 'tabletLandscape': (min-width: 50em), 'notebook': (min-width: 59.375em), 'desktop': (min-width: 75em), XL: (min-width: 93.75em)) !default' >> abstracts/_media-queries.sass
...
echo $'//	"notebook":  only screen and (min-width:  950px),\t\t\t\t\t\t\t//59.375em\t\t// 950px / 16px = 59.375em' >> abstracts/_media-queries.sass
echo $'//	"desktop":  only screen and (min-width: 1200px)\t\t\t\t\t\t\t\t\t//75em\t\t\t\t// 1200px / 16px = 75em' >> abstracts/_media-queries.sass
echo $'//	"XL":  only screen and (min-width: 1500px)\t\t\t\t\t\t\t\t\t//75em\t\t\t\t// 1500px / 16px = 75em' >> abstracts/_media-queries.sass
echo $'//) !default\n\n' >> abstracts/_media-queries.sass
